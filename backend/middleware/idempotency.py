    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            try:
                ...
                # If Redis is down, just proceed without idempotency safety
                print(f"WARNING: Idempotency skip due to Redis error: {e}")
                return await call_next(request)

            # 3. Process the actual request
            response = await call_next(request)
            
            # 4. Only cache successful or client-error responses
            if response.status_code < 500:
                try:
                    ...
                    # If body streaming or caching fails, return the original response if possible
                    # (Note: Starlette responses' body_iterator can only be consumed once)
                    return response
